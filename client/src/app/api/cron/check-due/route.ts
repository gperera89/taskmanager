...
//
// Deliberately thin: checkAndNotifyDueItems bundles the heartbeat stamp, the due reads, and
// the once-a-day retention sweeps into a single database operation on the common path (see
// api.ts's getCronSnapshot) to stay inside Prisma Postgres's free-tier operations budget.
export async function GET(request: Request) {
  ...
  return Response.json(await checkAndNotifyDueItems());
}
